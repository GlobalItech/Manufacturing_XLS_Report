...
class SDReportXls(ReportXlsx):
    ...
    def get_lines(self, date_from, date_to, product_categ, report_type, location, product, cat, warehouse):
        ...
        for product in product_ids:
            product_data=  self.env['stock.move'].search([('date_expected', '>=',date_from),('date_expected', '<=',date_to),('location_dest_id', 'in',array),('product_id', '=',product.id),('state','=','done')])
...
#         format3.set_align('center')
        ...
        if report_name == 'grand_production_summary_sd':
            sheet.merge_range(0, 0, 0, 2, 'Grand Production Summary Scheduled Date ', format11)
        ...
        if report_name == 'grand_production_summary_sd':
            for ware in warehouse:
                ...
                for lo in locations:
                    array1.append(lo.id)
                product_data=  self.env['mrp.production'].search([('date_planned_start', '>=',data['form']['date_from']),('date_planned_start', '<=',data['form']['date_to']),('location_dest_id', 'in',array1)])
                if product_data:
                ...
                #         
                        ...
                        if get_lines:
                            ...
                            product_row+=4
SDReportXls('report.manufacturing_sd_report_xls.sd_xls.xlsx','product.product')